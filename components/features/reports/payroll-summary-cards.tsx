...
export function PayrollSummaryCards({ payrollData, selectedPeriod }: PayrollSummaryCardsProps) {
  ...
  const payrollChange = totalPayroll > 0 ? ((totalPayroll - previousTotalPayroll) / previousTotalPayroll) * 100 : 0;
    ...
    <div className="*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4 px-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs lg:px-6 @xl/main:grid-cols-2 @5xl/main:grid-cols-4">
      ...
      <Card className="@container/card">
        <CardHeader>
          ...
          <CardAction>
            <Badge variant="outline">
              {payrollTrending ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
              {payrollTrending ? '+' : ''}{payrollChange.toFixed(1)}%
            </Badge>
          </CardAction>
        </CardHeader>
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
          <div className="line-clamp-1 flex gap-2 font-medium">
            ...
            {payrollTrending ? <TrendingUp className="size-4" /> : <TrendingDown className="size-4" />}
          </div>
      ...
      <Card className="@container/card">
        <CardHeader>
          ...
          <CardAction>
            <Badge variant="outline">
              {hoursTrending ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
              {hoursTrending ? '+' : ''}{hoursChange.toFixed(1)}%
            </Badge>
          </CardAction>
        </CardHeader>
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
      ...
      <Card className="@container/card">
        ...
        <CardFooter className="flex-col items-start gap-1.5 text-sm">
          <div className="line-clamp-1 flex gap-2 font-medium">
            Tips: {formatCurrency(totalTips)} • Bonuses: {formatCurrency(totalBonuses)}
