...
function getDepartmentRateInfo(
  user: User,
  departmentHours: Record<Department, number>
): DepartmentRateInfo[] {
...
function MobileRateCard({ rateInfo }: { rateInfo: DepartmentRateInfo }) {
  ...
  return (
    <Card
      className={`@container/card min-h-[120px] touch-manipulation ${hasWorkedHours ? 'bg-muted/30 border-hunks-green/20' : ''}`}
    >
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="font-medium text-base @[250px]/card:text-lg truncate">
          ...
          {departmentInfo.hasRoles ? (
                ...
                <div className="flex items-center gap-2">
                  <span className="font-mono text-sm font-medium">
                    {rateInfo.captainRate
                      ? formatCurrency(rateInfo.captainRate)
                      : 'N/A'}
                  </span>
                ...
                <div className="flex items-center gap-2">
                  <span className="font-mono text-sm font-medium">
                    {rateInfo.wingmanRate
                      ? formatCurrency(rateInfo.wingmanRate)
                      : 'N/A'}
                  </span>
            ...
            <div className="flex justify-between items-center p-2 bg-background rounded border">
              ...
              <div className="flex items-center gap-2">
                <span className="font-mono text-sm font-medium">
                  {rateInfo.singleRate
                    ? formatCurrency(rateInfo.singleRate)
                    : 'N/A'}
                </span>
                {hasWorkedHours && (
                  <Star className="h-4 w-4 text-hunks-green" />
        ...
        <>
          ...
          <TableCell className="text-right">
            <div className="flex items-center justify-end gap-1">
              ...
              {hasWorkedHours && <Star className="h-3 w-3 text-hunks-green" />}
            </div>
          </TableCell>
        </>
      )}
...
export function RateInformationPanel({
  ...
  const unworkedDepartments = rateInfos.filter(
    (info) => info.hoursWorked === 0
  );
  ...
  const averageRate =
    workedDepartments.length > 0
      ? workedDepartments.reduce((sum, info) => {
          const rate =
            info.singleRate ||
            (info.currentlyUsed === 'captain'
              ? info.captainRate
              : info.wingmanRate) ||
            0;
          return sum + rate;
                ...
                <Tooltip>
                  ...
                  <TooltipContent>
                    <div className="max-w-xs">
                      <p className="text-sm">
                        Rates marked with{' '}
                        <Star className="h-3 w-3 inline text-hunks-green" />{' '}
                        were used for departments where you worked hours this
                        period.
                      </p>
                    </div>
                  </TooltipContent>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="text-center">
                <div
                  className={`font-semibold ${isMobile ? 'text-xl' : 'text-lg'}`}
                >
                  {activeDepartments}
                </div>
                <div className="text-xs text-muted-foreground">
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <Badge variant="secondary">
                              {unworkedDepartments.length.toString()}
                            </Badge>
                            <span className="text-sm text-muted-foreground">
                              Departments where you didn&apos;t work this period
                            </span>
                          </div>
                        </Button>
                      </CollapsibleTrigger>
                          ...
                          >
                            <TableCell colSpan={3}>
                              <div className="flex items-center justify-center gap-2 py-2 text-sm text-muted-foreground">
                                ...
                                <span>
                                  Show {unworkedDepartments.length} unused
                                  departments
                                </span>
                              </div>
                            </TableCell>
