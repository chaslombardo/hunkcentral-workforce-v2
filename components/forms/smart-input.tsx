...
export function SmartInput({
  ...
  const validateValue = React.useCallback(async (inputValue: string) => {
    ...
    const sortedRules = [...validationRules].sort((a, b) => a.priority - b.priority);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (validateOnChange) {
      ...
      if (progressiveValidation) {
        if (validationState.hasBeenTouched || (validationState.hasBeenFocused && newValue.length > 0)) {
  ...
  const shouldShowValidation = React.useMemo(() => {
    ...
    return false;
  }, [showValidation, error, progressiveValidation, validationState.hasBeenTouched, validationState.hasBeenFocused, value.length]);

  // Determine input state
  const hasError = error || (shouldShowValidation && validationState.errors.length > 0);
  const hasSuccess = success || (shouldShowValidation && validationState.isValid && validationState.hasBeenTouched && value.length > 0);
  const hasWarning = shouldShowValidation && validationState.warnings.length > 0;
  ...
  return (
    <div className="grid gap-2">
      ...
      <div className="relative">
        <Input
          ...
          className={cn(
            ...
            hasSuccess && "border-hunks-green focus-visible:ring-hunks-green/20",
            hasWarning && "border-hunks-orange focus-visible:ring-hunks-orange/20",
      ...
      {/* Success Message */}
      {hasSuccess && success && (
        <Alert id={successId} className="border-hunks-green/20 bg-hunks-green/5">
...
export const commonValidationRules = {
  ...
  passwordStrengthWarning: (): ValidationRule => ({
    ...
    message: 'Consider using a stronger password with mixed case, numbers, and special characters',
    type: 'warning',
    priority: 5,
  }),
