  ...
  useEffect(() => {
    const registerServiceWorker = async () => {
      try {
        ...
        const events = ['click', 'keydown', 'scroll', 'touchstart', 'mousemove', 'focus', 'blur']
        ...
        try {
          ...
          if (!registration.active && !registration.waiting && !registration.installing) {
            ...
            if (process.env.NODE_ENV === 'development') {
              console.warn('SW: Registration in bad state, attempting recovery')
            }
    ...
    const handleRegistrationError = (error: unknown) => {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        console.warn('SW: App will continue without service worker functionality')
      }
    ...
    const handleSwUpdate = (event: CustomEvent) => {
      ...
      if (timeSinceLastCheck > minInterval) {
        ...
        const duration = sessionInfo.hasUnsavedWork ? 20000 : 
                        sessionInfo.hasActiveAuth ? 15000 : 10000
        ...
        toast({
          ...
          action: (
            ...
            >
              {sessionInfo.hasUnsavedWork ? 'Update Later' : 
               sessionInfo.hasActiveAuth ? 'Update Now' : 'Refresh'}
            </button>
          ),
        })
      }
    ...
    const handleUpdateAction = (sessionInfo: SessionInfo, registration?: ServiceWorkerRegistration) => {
      ...
      if (sessionInfo.hasUnsavedWork) {
        ...
        toast({
          ...
          description: 'Please save your work. The app will refresh in 10 seconds.',
          duration: 10000,
        })
    ...
    const performControlledRefresh = (registration?: ServiceWorkerRegistration) => {
      try {
    ...
    const shouldRegister = (): { register: boolean; reason: string } => {
      ...
      if (isDevelopment) {
        ...
        return { register: false, reason: 'development without explicit enable' }
      }
