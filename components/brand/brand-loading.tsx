...
const BrandLoading = React.memo(
  React.forwardRef<HTMLDivElement, BrandLoadingProps>(
    (
      {
        className,
        variant,
        ...
        switch (variant) {
          case 'spinner':
            return (
              <div
                className={cn(
                  brandLoadingVariants({ variant, size, color, className })
                )}
              >
      ...
      const accessibilityProps = React.useMemo(
        () => ({
          ...
          'aria-label':
            ariaLabel ||
            (text
              ? `${ariaLabels.status.loading}: ${text}`
              : ariaLabels.status.loading),
