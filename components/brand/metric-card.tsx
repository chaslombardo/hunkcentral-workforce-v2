...
const MetricCardSkeleton = React.memo(function MetricCardSkeleton({
  className,
}: {
  className?: string;
}) {
...
const MetricCard = React.memo(function MetricCard({
  ...
  const TrendIcon = React.useMemo(
    () => (change ? getTrendIcon(change.type) : null),
    [change]
  );
  ...
  const accessibilityProps = React.useMemo(() => {
    ...
    if (ariaLabel) {
      ...
      props['aria-label'] = ariaLabels.metric.value(
        title,
        displayValue,
        change
      );
  ...
  if (loading) {
    return (
      <MetricCardSkeleton className={cn(colorClasses.border, className)} />
    );
  }
        ...
        <CardHeader>
          ...
          {change && (
            <CardAction>
              <Badge
                variant="outline"
                className={cn('gap-1', colorClasses.badge)}
              >
                {TrendIcon && (
