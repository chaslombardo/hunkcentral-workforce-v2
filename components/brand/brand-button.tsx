  ...
  React.forwardRef<HTMLButtonElement, BrandButtonProps>(
    (
      ...
      const descriptionId = React.useRef(
        generateAccessibilityId('button-desc')
      );
      ...
      const accessibilityProps = React.useMemo(() => {
        ...
        if (ariaLabel) {
          ...
          switch (buttonState) {
            case 'loading':
              props['aria-label'] =
                loadingText || ariaLabels.button.loading(baseLabel);
              break;
            case 'success':
              props['aria-label'] =
                successText || ariaLabels.button.success(baseLabel);
              break;
            case 'error':
              props['aria-label'] =
                errorText || ariaLabels.button.error(baseLabel);
              break;
          }
        }
      ...
      const animationClasses = React.useMemo(() => {
        if (!respectReducedMotion) return 'transition-all duration-200';

        return getMicroInteractionClasses('buttonHover');
