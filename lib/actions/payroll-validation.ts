...
export async function validateEmployeePayroll(
  ...
  try {
    ...
    const payPeriod = await prisma.payPeriod
      .findUnique({
    ...
    if (!payPeriod) {
      return {
        success: false,
        error:
          'The requested pay period could not be found. It may have been deleted or you may not have access to it.',
    ...
    const approvedLogs = await prisma.dailyLog
      .findMany({
        where: {
          approvedAt: {
    ...
    const commissionEntries = await prisma.commissionEntry
      .findMany({
        where: {
          salesId: employeeId,
          status: 'matched',
    ...
    const commissionsForValidation: CommissionEntry[] = commissionEntries.map(
      (commission) => ({
        ...commission,
...
export async function submitDiscrepancyReport(
  ...
  try {
    ...
    const report = await prisma.discrepancyReport
      .create({
        data: {
          employeeId,
          payPeriodId,
...
export async function getEmployeeDiscrepancyReports(
  ...
  try {
    ...
    const whereClause: { employeeId: string; payPeriodId?: string } = {
      employeeId,
    };
    if (payPeriodId) {
    ...
    const reports = await prisma.discrepancyReport
      .findMany({
        where: whereClause,
        include: {
          employee: {
      ...
      .catch(() => {
        ...
        throw new Error(
          'Unable to load discrepancy reports. Please try again.'
        );
      });
