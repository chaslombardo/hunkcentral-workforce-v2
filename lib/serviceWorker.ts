...
export class ServiceWorkerManager {
  ...
  private validateEnvironment(): { valid: boolean; reason: string } {
    ...
    if (swDisabled) {
      return {
        valid: false,
        reason: 'explicitly disabled via NEXT_PUBLIC_SW_DISABLED',
      };
    }
  ...
  private async cleanupProblematicRegistrations(): Promise<void> {
    try {
      ...
      if (
        ...
        this.isRegistrationProblematic(existingRegistration)
      ) {
  ...
  private isRegistrationProblematic(
  ...
  private handleUpdateWithSessionProtection(): void {
    ...
    const getNotificationDelay = () => {
      ...
      if (sessionAnalysis.hasActiveAuth && sessionAnalysis.hasRecentActivity)
  ...
  private analyzeUserSession() {
    try {
      ...
      const sessionCriticality = hasUnsavedWork
        ? 'critical'
        : hasActiveAuth && hasRecentActivity
          ? 'high'
          : hasActiveAuth
            ? 'medium'
            : 'low';
  ...
  private async performHealthCheck(): Promise<void> {
    ...
    try {
      ...
      this.registration.active.postMessage({ type: 'SW_HEALTH_CHECK' }, [
        channel.port2,
      ]);
  ...
  private handleHealthCheckFailure(): void {
    ...
    if (isDevelopment) {
      console.warn(
        'SW: Health check failed, service worker may be unresponsive'
      );
    }
  ...
  private async handleRegistrationFailure(
    ...
    if (isDevelopment) {
      console.warn(
        `SW: Registration failed (attempt ${this.registrationAttempts}/${this.maxRegistrationAttempts})`,
        {
          error: errorMessage,
          type: errorType,
    ...
    if (typeof window !== 'undefined') {
      ...
      sessionStorage.setItem(
        'sw-error-attempts',
        this.registrationAttempts.toString()
      );
    }
  ...
  private categorizeRegistrationError(error: unknown): string {
    ...
    if (
      ...
      errorMessage.includes('implement')
    ) {
  ...
  private notifyUpdate(
    sessionAnalysis?: ReturnType<typeof this.analyzeUserSession>
  ): void {
  ...
  checkForActiveSession(): boolean {
    const sessionAnalysis = this.analyzeUserSession();
    return (
      sessionAnalysis.hasActiveAuth ||
      sessionAnalysis.hasUnsavedWork ||
      sessionAnalysis.hasRecentActivity
    );
  }
}
