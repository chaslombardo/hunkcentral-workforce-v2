...
export function analyzeComponentVariants(): ComponentVariantAnalysis[] {
  ...
  const componentVariants: Record<string, string[]> = {
    ...
    'StatusIndicator': [
      'pending', 'approved', 'matched', 'rejected', 'open', 'locked', 
      'closed', 'active', 'inactive', 'success', 'warning', 'error', 
      'info', 'processing', 'draft', 'submitted', 'completed'
    ],
    'BrandLoading': [
      'spinner', 'dots', 'pulse'
    ],
    'MetricCard': [
      'green', 'orange', 'blue', 'purple', 'neutral'
    ],
    'SmartInput': [
      'default', 'email', 'numeric', 'tel', 'url', 'search'
    ],
    'FormFeedback': [
  ...
  for (const [componentName, allVariants] of Object.entries(componentVariants)) {
    ...
    if (unusedVariants.length > allVariants.length * 0.5) {
      recommendations.push('Consider creating a minimal version with only used variants');
    }
    
    if (usedVariants.length === 1) {
      recommendations.push('Only one variant is used - consider simplifying the component');
    }
    
    if (unusedVariants.length > 0) {
      recommendations.push(`Remove unused variants: ${unusedVariants.slice(0, 3).join(', ')}${unusedVariants.length > 3 ? '...' : ''}`);
    }
...
export function generateOptimizationReport(): {
  ...
  const summary = {
    ...
    componentsWithUnusedVariants: analyses.filter(a => a.unusedVariants.length > 0).length,
    totalUnusedVariants: analyses.reduce((sum, a) => sum + a.unusedVariants.length, 0),
    averageUsagePercentage: analyses.reduce((sum, a) => sum + a.usagePercentage, 0) / analyses.length
  };
  ...
  if (summary.averageUsagePercentage < 50) {
    recommendations.push('Consider creating minimal component variants to reduce bundle size');
  }
  
  if (summary.totalUnusedVariants > 10) {
    recommendations.push('High number of unused variants detected - implement tree-shaking optimizations');
  }
  
  // Component-specific recommendations
  const lowUsageComponents = analyses.filter(a => a.usagePercentage < 30);
  if (lowUsageComponents.length > 0) {
    recommendations.push(`Components with low variant usage: ${lowUsageComponents.map(c => c.componentName).join(', ')}`);
  }
...
export function createOptimizedVariants(componentName: string): {
  ...
  const analysis = analyzeComponentVariants().find(a => a.componentName === componentName);
...
export function logOptimizationReport(): void {
  ...
  if (report.componentAnalyses.length > 0) {
    console.warn('📋 Component Analysis:', 
