...
jobQueue.registerProcessor({
  ...
  handler: async (payload) => {
    const typedPayload = payload as { userId?: string; payPeriodId?: string };
    await calculatePrecomputedMetrics('dashboard', typedPayload);
  },
});
jobQueue.registerProcessor({
  type: JOB_TYPES.COMPUTE_PAYROLL_METRICS,
  handler: async (payload) => {
    const typedPayload = payload as { payPeriodId: string; userId?: string };
    await calculatePrecomputedMetrics('payroll', typedPayload);
  },
});
jobQueue.registerProcessor({
  ...
  handler: async (payload) => {
    ...
    await calculatePrecomputedMetrics('labor_costs', typedPayload);
  },
});
jobQueue.registerProcessor({
  ...
  handler: async (payload) => {
    const typedPayload = payload as { userId?: string; payPeriodId?: string };
    await calculatePrecomputedMetrics('commission', typedPayload);
  },
});
jobQueue.registerProcessor({
  ...
  handler: async (payload) => {
    const typedPayload = payload as { userId: string; payPeriodId?: string };
    await calculatePrecomputedMetrics('user_performance', typedPayload);
  },
});
jobQueue.registerProcessor({
  type: JOB_TYPES.INVALIDATE_CACHE,
  handler: async (payload) => {
    const { metricType, entityId } = payload as {
      metricType?: string;
      entityId?: string;
    };
    await invalidateMetricsCache(metricType, entityId);
  },
});
jobQueue.registerProcessor({
...
export async function refreshAllMetrics(): Promise<void> {
  console.warn('Starting full metrics refresh...');
  ...
  console.warn(`Queued ${jobs.length} metric computation jobs`);
}
