...
export async function logProductionError(
  ...
  try {
    ...
    if (existingError) {
      ...
      errorLog.firstOccurrence =
        (existingChanges.firstOccurrence as string) || errorLog.firstOccurrence;
    } else {
...
async function sendCriticalErrorAlert(
  ...
  try {
    ...
    process.stderr.write(
      `ALERT_SYSTEM_FAILURE: ${JSON.stringify({
        error:
