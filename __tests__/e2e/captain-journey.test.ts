...
test.describe('Captain Mobile Journey', () => {
  ...
  test('should complete full captain workflow on mobile', async ({ page }) => {
    ...
    await page.fill(
      '[data-testid="client-name-input-0"]',
      'Mobile Test Client'
    );
    ...
    await page.fill(
      '[data-testid="client-name-input-1"]',
      'Mobile Test Client 2'
    );
    ...
    await expect(
      page.locator('[data-testid="section-total-revenue"]')
    ).toContainText('$1,400');
    await expect(
      page.locator('[data-testid="section-total-tips"]')
    ).toContainText('$140');
    await expect(page.locator('[data-testid="tips-per-hunk"]')).toContainText(
      '$70.00'
    );

    // Labor cost percentage should be calculated
    await expect(
      page.locator('[data-testid="labor-cost-percentage"]')
    ).toBeVisible();
    ...
    await expect(
      page.locator('[data-testid="log-status-submitted"]')
    ).toBeVisible();
    await expect(page.locator('[data-testid="log-revenue"]')).toContainText(
      '$1,400'
    );
  });
  ...
  test('should handle form validation errors gracefully', async ({ page }) => {
    ...
    ).toContainText('Cannot submit empty log');
    ...
    ).toContainText('Client name is required');
  ...
  test('should work correctly on different mobile screen sizes', async ({
    page,
  }) => {
  ...
  test('should maintain form state during auto-save', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="job-id-input-0"]')).toHaveValue(
      'AUTOSAVE-TEST'
    );
    await expect(
      page.locator('[data-testid="client-name-input-0"]')
    ).toHaveValue('Auto Save Client');

    // Recently entered data might not be saved yet, but basic structure should be preserved
    await expect(
      page.locator('[data-testid="section-junk-checkbox"]')
    ).toBeChecked();
  });

  test('should provide appropriate keyboard types for mobile inputs', async ({
    page,
  }) => {
    // Login
    await page.fill('[data-testid="email-input"]', 'captain@test.com');
    ...
    await expect(
      page.locator('[data-testid="revenue-input-0"]')
    ).toHaveAttribute('inputmode', 'decimal');
    await expect(page.locator('[data-testid="tips-input-0"]')).toHaveAttribute(
      'inputmode',
      'decimal'
    );

    await page.click('[data-testid="add-hunk-button"]');
    await expect(page.locator('[data-testid="hours-input-0"]')).toHaveAttribute(
      'inputmode',
      'decimal'
    );
    ...
    await expect(
      page.locator('[data-testid="job-id-input-0"]')
    ).toHaveAttribute('type', 'text');
  });
});
...
test.describe('Captain Desktop Journey', () => {
  ...
  test('should adapt interface for desktop use', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="desktop-form-layout"]')
    ).toBeVisible();
    ...
    await expect(
      page.locator('[data-testid="calculations-sidebar"]')
    ).toBeVisible();
  });
