...
test.describe('Theme & UX Improvements E2E Tests', () => {
  ...
  test.describe('Brand Visual Identity', () => {
    test('displays consistent brand colors throughout the application', async ({
      page,
    }) => {
      // Login
      ...
      await expect(primaryButtons.first()).toHaveCSS(
        'background-color',
        'rgb(2, 105, 55)'
      ); // #026937
      ...
      if ((await secondaryButtons.count()) > 0) {
        await expect(secondaryButtons.first()).toHaveCSS(
          'background-color',
          'rgb(234, 114, 0)'
        ); // #ea7200
      }
      ...
      if ((await approvedStatus.count()) > 0) {
        await expect(approvedStatus.first()).toHaveCSS(
          'background-color',
          'rgb(2, 105, 55)'
        );
      }
      ...
      if ((await pendingStatus.count()) > 0) {
        await expect(pendingStatus.first()).toHaveCSS(
          'background-color',
          'rgb(234, 114, 0)'
        );
      }
    ...
    test('displays enhanced metric cards with brand styling', async ({
      page,
    }) => {
      ...
      await expect(metricCards.first()).toHaveCSS(
        'border-left-color',
        'rgb(2, 105, 55)'
      );
  ...
  test.describe('Enhanced Navigation', () => {
    test('displays smart breadcrumbs with proper navigation', async ({
      page,
    }) => {
      // Login
      await page.fill('[data-testid="email-input"]', 'captain@test.com');
    ...
    test('shows unified mobile navigation on small screens', async ({
      page,
    }) => {
      // Set mobile viewport
      ...
      const activeButton = page.locator(
        '[data-testid="mobile-nav-logs"][data-active="true"]'
      );
    ...
    test('displays navigation badges for pending items', async ({ page }) => {
      ...
      const pendingLogsBadge = page.locator(
        '[data-testid="nav-badge-pending-logs"]'
      );
      if ((await pendingLogsBadge.count()) > 0) {
        await expect(pendingLogsBadge).toBeVisible();
        await expect(pendingLogsBadge).toHaveCSS(
          'background-color',
          'rgb(234, 114, 0)'
        ); // Orange
      }

      const draftLogsBadge = page.locator(
        '[data-testid="nav-badge-draft-logs"]'
      );
    ...
    test('adapts navigation for different user roles', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="nav-commission"]')
      ).toBeVisible();
      ...
      await expect(
        page.locator('[data-testid="nav-pay-periods"]')
      ).toBeVisible();
    });
  });
  ...
  test.describe('Enhanced Form Interactions', () => {
    ...
    test('handles mobile form optimizations', async ({ page }) => {
      ...
      if ((await numberInputs.count()) > 0) {
        await expect(numberInputs.first()).toHaveAttribute(
          'inputmode',
          'decimal'
        );
      }
  ...
  test.describe('Accessibility Features', () => {
    test('supports keyboard navigation throughout the application', async ({
      page,
    }) => {
      // Login
      await page.fill('[data-testid="email-input"]', 'captain@test.com');
    ...
    test('provides proper ARIA labels and screen reader support', async ({
      ...
      if ((await statusIndicators.count()) > 0) {
        await expect(statusIndicators.first()).toHaveAttribute(
          'role',
          'status'
        );
      }
    ...
    test('respects motion preferences', async ({ page }) => {
      ...
      if ((await transitionElements.count()) > 0) {
        await expect(transitionElements.first()).toHaveCSS(
          'transition-duration',
          '0s'
        );
      }
  ...
  test.describe('Cross-Browser Compatibility', () => {
    test('works consistently across different browsers', async ({
      page,
      browserName,
    }) => {
      // Login
