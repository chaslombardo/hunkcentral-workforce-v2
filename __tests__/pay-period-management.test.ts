...
describe('Pay Period Management', () => {
  ...
  describe('createPayPeriod', () => {
    ...
    it('should reject overlapping pay periods', async () => {
      ...
      vi.mocked(prisma.payPeriod.findFirst).mockResolvedValue(
        overlappingPeriod as any
      );
  ...
  describe('updatePayPeriodStatus', () => {
    it('should update status from open to locked', async () => {
      ...
      vi.mocked(prisma.payPeriod.findUnique).mockResolvedValue(
        existingPeriod as any
      );
      vi.mocked(prisma.payPeriod.update).mockResolvedValue(
        updatedPeriod as any
      );

      const result = await updatePayPeriodStatus({
        id: 'period-1',
        status: 'locked',
      });
  ...
  describe('deletePayPeriod', () => {
    it('should delete open pay periods', async () => {
      ...
      vi.mocked(prisma.payPeriod.delete).mockResolvedValue(
        existingPeriod as any
      );
  ...
  describe('canModifyDataForDate', () => {
    ...
    it('should handle database errors gracefully', async () => {
      vi.mocked(prisma.payPeriod.findFirst).mockRejectedValue(
