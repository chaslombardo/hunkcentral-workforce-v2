...
describe('TipsDetailView', () => {
  ...
  it('filters tips by job type correctly', () => {
    ...
    const filterSelect = filterSelects.find(
      (select) => select.getAttribute('aria-expanded') === 'false'
    );
  ...
  it('calculates job type averages correctly', () => {
    ...
    // Move jobs: 20 / 1 = 20
    expect(screen.getByTestId('job-type-comparison')).toHaveTextContent(
      '$35.00 / $20.00'
    );
  });
  ...
  it('shows tip distribution formula explanation', () => {
    ...
    expect(screen.getByTestId('tip-formula')).toHaveTextContent(
  ...
  it('displays empty state when no tips match filters', () => {
    ...
    expect(
      screen.getByText('No tips match your current filters.')
    ).toBeInTheDocument();
  });
  ...
  it('renders tooltips for tip calculation explanations', () => {
    ...
    const tooltipButtons = buttons.filter(
      (button) =>
  ...
  it('handles empty tips array gracefully', () => {
    ...
    expect(
      screen.getByText('No tips recorded for this period.')
    ).toBeInTheDocument();
  });
});
