...
describe('PayPeriodAnalysis', () => {
  ...
  it('renders the component with header and controls', () => {
    ...
    expect(screen.getByText('Compare performance across pay periods and track trends')).toBeInTheDocument();
  ...
  it('displays error message when error prop is provided', () => {
    ...
    expect(screen.getByText(`${errorMessage}. Showing cached analysis data.`)).toBeInTheDocument();
  });
  ...
  it('allows switching between chart views', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Department Hour Distribution')).toBeInTheDocument();
    }, { timeout: 3000 });
  ...
  it('displays performance insights', () => {
    ...
    expect(screen.getByText('AI-powered insights based on your work patterns and performance')).toBeInTheDocument();
  ...
  it('displays insight recommendations when available', () => {
    ...
    expect(screen.getByText('💡 Keep up the excellent customer service!')).toBeInTheDocument();
    expect(screen.getByText('💡 Continue focusing on efficient job completion')).toBeInTheDocument();
    expect(screen.getByText('💡 Monitor hours to optimize pay structure')).toBeInTheDocument();
  });
  ...
  it('applies correct styling for different insight types', () => {
    ...
    const insightCards = screen.getAllByText(/Tips Performance Improved|Department Mix Changed|Labor Efficiency Bonus|Approaching Overtime/);
