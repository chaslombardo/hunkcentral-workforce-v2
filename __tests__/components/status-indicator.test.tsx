...
import { StatusIndicator, StatusIndicators, getStatusType } from '@/components/brand/status-indicator';
...
describe('StatusIndicator', () => {
  ...
  it('applies correct size classes', () => {
    ...
    expect(screen.getByTestId('status-indicator')).toHaveClass('text-xs', 'h-5');

    rerender(<StatusIndicator status="pending" size="md" />);
    expect(screen.getByTestId('status-indicator')).toHaveClass('text-sm', 'h-6');

    rerender(<StatusIndicator status="pending" size="lg" />);
    expect(screen.getByTestId('status-indicator')).toHaveClass('text-sm', 'h-8');
  });
  ...
  describe('Brand color mapping', () => {
    it('applies brand green colors for success states', () => {
      const successStatuses = ['approved', 'matched', 'success', 'completed', 'active', 'open'];
      
      successStatuses.forEach(status => {
        const { unmount } = render(<StatusIndicator status={status as any} />);
        expect(screen.getByTestId('status-indicator')).toHaveClass('bg-hunks-green-50', 'text-hunks-green-700');
        unmount();
      });
    });
    ...
    it('applies brand orange colors for warning states', () => {
      ...
      warningStatuses.forEach(status => {
        const { unmount } = render(<StatusIndicator status={status as any} />);
        expect(screen.getByTestId('status-indicator')).toHaveClass('bg-hunks-orange-50', 'text-hunks-orange-700');
        unmount();
      });
    });
    ...
    it('applies red colors for error states', () => {
      ...
      errorStatuses.forEach(status => {
        const { unmount } = render(<StatusIndicator status={status as any} />);
        expect(screen.getByTestId('status-indicator')).toHaveClass('bg-red-50', 'text-red-700');
        unmount();
      });
    });
  });
  ...
  describe('Custom styling', () => {
    it('accepts custom className', () => {
      ...
      expect(screen.getByTestId('status-indicator')).toHaveClass('custom-class');
    });
...
describe('StatusIndicators pre-configured components', () => {
  ...
  it('renders Approved component with correct props', () => {
    ...
    expect(screen.getByTestId('status-indicator')).toHaveClass('bg-hunks-green-50');
  });
