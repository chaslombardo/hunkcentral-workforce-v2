...
vi.mock(
  '@/components/features/reports/payroll-breakdown/payroll-validation-panel',
  () => ({
    PayrollValidationPanel: ({ validationResult }: any) => (
      <div data-testid="payroll-validation-panel">
        Validation Panel:{' '}
        {validationResult ? 'Has validation data' : 'No validation data'}
      </div>
    ),
  })
);
...
describe('MyPayrollView Enhanced Component', () => {
  ...
  it('should have responsive tab navigation', async () => {
    ...
    ).toBeInTheDocument(); // Pay History tab
    expect(
  ...
  it('should switch between tabs correctly', async () => {
    ...
    expect(screen.getByRole('tab', { name: /breakdown/i })).toHaveAttribute(
      'data-state',
      'active'
    );
  ...
  it('should show mobile-optimized pay history', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('tab', { name: /pay history/i })).toHaveAttribute(
        'data-state',
        'active'
      );
    });
  ...
  it('should handle validation tab correctly', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('tab', { name: /validation/i })).toHaveAttribute(
        'data-state',
        'active'
      );
    });
