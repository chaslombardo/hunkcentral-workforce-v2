...
describe('Production Service Worker Caching Strategy', () => {
  ...
  describe('Caching Strategy', () => {
    it('should cache only essential static assets', async () => {
      ...
      mockFetch.mockImplementation((url) => {
        if (typeof url === 'string' && essentialAssets.some(asset => url.includes(asset))) {
  ...
  describe('Graceful Degradation', () => {
    ...
    it('should handle cache failures gracefully', async () => {
      ...
      try {
        await mockCache.put(new Request('http://localhost:3000/test'), new Response('test'))
      } catch (error) {
        expect(error).toBeInstanceOf(Error)
  ...
  describe('Production Safety Features', () => {
    it('should use shorter cache durations in production', () => {
      ...
      expect(developmentConfig.staticMaxAge).toBeLessThan(productionConfig.staticMaxAge)
      expect(developmentConfig.runtimeMaxAge).toBeLessThan(productionConfig.runtimeMaxAge)
      expect(developmentConfig.apiMaxAge).toBeLessThan(productionConfig.apiMaxAge)
    })
