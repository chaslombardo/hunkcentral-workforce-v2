...
export function useNavigationState() {
  ...
  return {
    ...
    // Helper functions
    getBadgeCount: (badgeId: string) =>
      navigation.state.badges[badgeId]?.count || 0,
    getBadgeType: (badgeId: string) =>
      navigation.state.badges[badgeId]?.type || 'pending',
    hasBadge: (badgeId: string) =>
      !!navigation.state.badges[badgeId] &&
      navigation.state.badges[badgeId].count > 0,

    // Total counts for dashboard/summary views
    getTotalPendingLogs: () =>
      navigation.state.badges['logs-review']?.count || 0,
    getTotalDraftLogs: () => navigation.state.badges['logs-draft']?.count || 0,
    getTotalPendingCommissions: () =>
      navigation.state.badges['commission-pending']?.count || 0,

    // Get all badges of a specific type
    getBadgesByType: (type: 'pending' | 'warning' | 'info') =>
      Object.values(navigation.state.badges).filter(
