...
import { CaptainLogFormV2 } from '@/components/features/logs/captain-log-form-v2';
...
export const metadata: Metadata = {
  title: 'Create Daily Log (V2) - HUNKCentral',
  description: 'Create a new daily work log - Grid Time Tracking Version',
};

export default async function CreateLogV2Page() {
  let session;
  let userAgent: string | undefined;
  let currentUrl: string = '/logs/create-v2';

  try {
    // Get request headers for error context
    const headersList = await headers();
    userAgent = headersList.get('user-agent') || undefined;
    currentUrl = headersList.get('x-url') || '/logs/create-v2';
      ...
      <div className="container mx-auto py-6">
        <div className="mb-6">
          <h1 className="text-3xl font-bold tracking-tight">
            Create Daily Log - Grid Time Tracking
          </h1>
          <p className="text-muted-foreground">
            Visual grid interface for efficient team time allocation across all job types.
          </p>
          <div className="mt-2 flex items-center gap-2">
            ...
            <span className="text-sm text-muted-foreground">
              Team-focused grid with visual time tracking
            </span>
          </div>
        </div>
