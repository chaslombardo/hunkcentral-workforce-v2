...
async function testTouchTargets(device) {
  ...
  if (device.width < 375) {
    return {
      status: 'warning',
      message: 'Small screen may have touch target issues',
    };
  }
...
async function testResponsive(device) {
  ...
  if (aspectRatio < 0.5 || aspectRatio > 2) {
    return {
      status: 'warning',
      message: 'Unusual aspect ratio may cause layout issues',
    };
  }
...
async function testPerformance(device) {
  ...
  if (device.width > 1024) {
    return {
      status: 'passed',
      message: 'Good performance expected on large screens',
    };
  } else if (device.width < 375) {
    return {
      status: 'warning',
      message: 'Performance may be impacted on small screens',
    };
  }
...
function generateMobileReport() {
  const total =
    ...
    testResults.summary.warnings;
  const passRate =
    total > 0 ? ((testResults.summary.passed / total) * 100).toFixed(1) : 0;
