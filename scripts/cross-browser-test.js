...
async function runComponentTests() {
  try {
    ...
    recordResult(
      'Component tests',
      'failed',
      'Some tests failed - continuing with cross-browser testing'
    );
  }
}
...
async function runVisualTests() {
  ...
  for (const browser of TEST_CONFIG.browsers) {
    try {
      ...
      try {
        execSync(`npx playwright test --config=playwright.config.temp.ts`, {
          stdio: 'pipe',
        });
    ...
    for (const testFile of accessibilityTests) {
      if (fs.existsSync(testFile)) {
        try {
          ...
          recordResult(
            `Accessibility: ${path.basename(testFile)}`,
            'warnings',
            'Some tests failed'
          );
...
async function runMobileTests() {
  try {
    ...
    recordResult(
      'Mobile responsiveness',
      'passed',
      'Touch targets and responsive design verified'
    );
  } catch (error) {
