...
async function runTestPhase(phase) {
  ...
  log(`Critical: ${phase.critical ? 'YES' : 'NO'}`, phase.critical ? 'red' : 'green');
  ...
  try {
    ...
    result = result || { status: 'passed', message: 'Phase completed successfully' };
...
async function runPreflightChecks() {
  ...
  checks.push({ name: 'Node.js version', value: nodeVersion, status: 'passed' });
  ...
  if (fs.existsSync('.next')) {
    ...
    checks.push({ name: 'Application built', status: 'warning', message: 'No build found' });
  }
  
  // Display results
  checks.forEach(check => {
    const statusColor = check.status === 'passed' ? 'green' : 
                       check.status === 'warning' ? 'yellow' : 'red';
    const statusIcon = check.status === 'passed' ? '✓' : 
                      check.status === 'warning' ? '⚠' : '✗';
    
    log(`  ${statusIcon} ${check.name}${check.value ? `: ${check.value}` : ''}`, statusColor);
  });
...
async function runCodeQuality() {
  ...
  try {
    ...
    results.push({ name: 'ESLint', status: 'warning', message: 'Linting issues found' });
  ...
  try {
    ...
    results.push({ name: 'TypeScript', status: 'failed', message: 'Type errors found' });
  ...
  try {
    ...
    try {
      ...
      const passRate = testResults.numTotalTests > 0 ? 
        ((testResults.numPassedTests / testResults.numTotalTests) * 100).toFixed(1) : 0;
      
      log(`  Tests: ${testResults.numPassedTests}/${testResults.numTotalTests} passed (${passRate}%)`, 'blue');
...
async function runAccessibilityTests() {
  ...
  if (warnings.length > 0) {
    return { status: 'warning', message: `${warnings.length} accessibility issues found` };
  }
...
async function runIntegrationTests() {
  ...
  const warnings = results.filter(r => r.status === 'warning');
  if (warnings.length > 0) {
    return { status: 'warning', message: `${warnings.length} integration issues found` };
  }
...
function generateFinalReport(duration) {
  ...
  const { total, passed, failed, warnings, critical_failures } = testSuite.summary;
  ...
  log(`Critical Failures: ${critical_failures}`, critical_failures > 0 ? 'red' : 'green');
