...
const isDevelopment = (() => {
  try {
    ...
    const isDevPort = port && (
      ...
      port === '8080' || port === '5000' || port === '4000' ||
      port === '5173' || port === '4173' // Vite dev/preview ports
    )
...
self.addEventListener('install', (event) => {
  logInDevelopment('Installing service worker', SW_VERSION, `(${isDevelopment ? 'development' : 'production'} mode)`)
  ...
  event.waitUntil(
    performInstallation()
      ...
      .catch((error) => {
        logError('Installation failed, continuing with graceful degradation', error)
...
async function performInstallation() {
  ...
  if (isDevelopment) {
    ...
    logInDevelopment(`Installation tasks completed: ${successful}/${results.length} successful`)
  }
...
async function cacheEssentialAssets() {
  try {
    ...
    const cachePromises = ESSENTIAL_STATIC_ASSETS.map(async (asset) => {
      try {
        ...
        if (response.ok) {
          ...
          headers.set('sw-environment', isDevelopment ? 'development' : 'production')
          ...
          logInDevelopment(`✗ Failed to cache ${asset}: HTTP ${response.status}`)
    ...
    logInDevelopment(`Static assets: ${successful}/${ESSENTIAL_STATIC_ASSETS.length} cached`)
...
async function cleanupStaticCache(cache) {
  try {
    ...
    if (requests.length > CACHE_CONFIG.maxStaticEntries) {
      ...
      const toDelete = sortedRequests.slice(0, requests.length - CACHE_CONFIG.maxStaticEntries)
      
      await Promise.all(
        toDelete.map(({ request }) => cache.delete(request))
      )
      
      logInDevelopment(`Cleaned up ${toDelete.length} old static cache entries`)
    }
  } catch (error) {
...
self.addEventListener('activate', (event) => {
  logInDevelopment('Activating service worker', SW_VERSION, `(${isDevelopment ? 'development' : 'production'} mode)`)
...
async function performActivation() {
  ...
  if (isDevelopment) {
    const successful = results.filter(r => r.status === 'fulfilled').length
    logInDevelopment(`Activation tasks completed: ${successful}/${results.length} successful`)
...
async function cleanupOldCaches() {
  try {
    ...
    const currentCaches = [STATIC_CACHE_NAME, RUNTIME_CACHE_NAME, OFFLINE_CACHE_NAME]
    ...
    if (oldCaches.length > 0) {
      ...
      const cleanupPromises = oldCaches.map(async (cacheName) => {
        try {
          ...
          logInDevelopment(`✗ Failed to delete cache ${cacheName}:`, error.message)
      ...
      const successful = results.filter(r => 
        r.status === 'fulfilled' && r.value.success && r.value.deleted
      ).length
      
      logInDevelopment(`Cache cleanup: ${successful}/${oldCaches.length} caches deleted`)
    } else {
  ...
  try {
    if (isDevelopment) {
      ...
      if (clients.length === 0) {
        ...
        logInDevelopment(`⏸ Skipped claiming (${clients.length} active sessions)`)
        ...
        clients.forEach(client => {
          try {
            ...
            logInDevelopment('Failed to notify client of update:', error.message)
          }
        })
      }
    }
...
async function notifyClientsOfActivation() {
  try {
    ...
    if (clients.length > 0) {
      ...
      logInDevelopment(`✓ Notified ${successful}/${clients.length} clients of activation`)
    } else {
...
function shouldCacheAsset(url) {
  try {
    ...
    if (isProduction) {
      ...
      const isStaticAsset = /\.(png|jpg|jpeg|gif|svg|ico|webp|woff|woff2|ttf|eot)$/.test(pathname)
...
async function handleApiRequest(request, url) {
  const isSafeRoute = SAFE_API_ROUTES.some(route => url.pathname.startsWith(route))
...
async function getCachedApiResponse(request) {
  try {
    ...
    if (cachedResponse && isCacheValid(cachedResponse, CACHE_CONFIG.apiMaxAge)) {
      return cachedResponse
    }
...
function isCacheValid(response, maxAge = null) {
  try {
    ...
    const maxCacheAge = maxAge || parseInt(response.headers.get('sw-max-age') || '0', 10) || CACHE_CONFIG.runtimeMaxAge
...
async function cleanupRuntimeCache(cache) {
  try {
    ...
    if (requests.length > CACHE_CONFIG.maxRuntimeEntries) {
      ...
      sortedRequests.sort((a, b) => a.timestamp - b.timestamp)
      const toDelete = sortedRequests.slice(0, requests.length - CACHE_CONFIG.maxRuntimeEntries)
      
      await Promise.all(
        toDelete.map(({ request }) => cache.delete(request).catch(() => {}))
      )
      
      logInDevelopment(`Cleaned up ${toDelete.length} old runtime cache entries`)
    }
  } catch (error) {
...
async function handleStaticAsset(request, url) {
  ...
  try {
    ...
    if (cachedResponse && isCacheValid(cachedResponse, CACHE_CONFIG.staticMaxAge)) {
...
async function getCachedStaticAsset(request, allowStale = false) {
  try {
    ...
    if (cachedResponse) {
      if (allowStale || isCacheValid(cachedResponse, CACHE_CONFIG.staticMaxAge)) {
        return cachedResponse
      }
...
async function cleanupExpiredCaches(cacheName, maxAge) {
  try {
    ...
    if (expiredRequests.length > 0) {
      ...
      logInDevelopment(`Cleaned up ${expiredRequests.length} expired entries from ${cacheName}`)
    }
    ...
    for (const cacheName of cacheNames) {
      try {
        ...
        cacheInfo[cacheName] = {
          ...
          isActive: [STATIC_CACHE_NAME, RUNTIME_CACHE_NAME, OFFLINE_CACHE_NAME].includes(cacheName)
        }
      } catch {
        cacheInfo[cacheName] = {
          entryCount: 0,
...
async function handleCacheClear() {
  try {
    ...
    const deletePromises = cacheNames.map(async (cacheName) => {
      try {
        const deleted = await caches.delete(cacheName)
        logInDevelopment(`Cache ${cacheName}: ${deleted ? 'deleted' : 'not found'}`)
        return { cacheName, deleted, success: true }
      } catch (error) {
        logError(`Failed to delete cache ${cacheName}`, error)
        return { cacheName, deleted: false, success: false, error: error.message }
      }
    })
    ...
    logInDevelopment(`Cache clearing: ${successful}/${cacheNames.length} caches deleted`)
