...
HUNKCentral is a digital workforce management system that replaces paper logs and Excel spreadsheets for College Hunks Hauling Junk & Moving. The system enables captains to submit daily work logs, managers to review, edit and approve them, sales staff to submit entries for commissions, auto matching by job id number from captain end of day log submissions, then track commissions, and administrators to generate payroll reports showing various reports that breakdown each employees pay, hours, tips, bonuses, salaries, and commission. Built as a modern next.js web application with shadcn/ui (New York theme) and include College HUNKS Hauling Junk and Moving Branding and Colors, the system focuses on simplicity and mobile-first design to streamline operations for field workers and office staff. Web app is to be used at office when hunks return from the days to submit log sheets, commissions entries are submitted by sales consultants at any time or location, and managers review and vet log submissions and are routinely editing them to make sure the hours reported for each employee matches humanity time punches and to verify job details are reported accurately such as revenue numbers and upsell numbers both match our franchise website "Hunkware" (which is where the job details, quotes, schedules, client info, and billing/payments are handled).
