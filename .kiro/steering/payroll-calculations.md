...
function calculateSection(section: LogSection, goalPercent: number) {
  const laborCost = section.entries.reduce((acc, e) => acc + e.hours * e.rate, 0);
  const laborPercent = section.revenue === 0 ? 0 : laborCost / section.revenue;
  const bonus = laborPercent < goalPercent ? (goalPercent - laborPercent) * section.revenue : 0;
  const tipsPerHunk = section.entries.length === 0 ? 0 : section.tips / section.entries.length;
